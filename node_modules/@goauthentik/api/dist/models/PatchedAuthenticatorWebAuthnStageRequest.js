"use strict";
/* tslint:disable */
/* eslint-disable */
/**
 * authentik
 * Making authentication simple.
 *
 * The version of the OpenAPI document: 2025.6.3
 * Contact: hello@goauthentik.io
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.instanceOfPatchedAuthenticatorWebAuthnStageRequest = instanceOfPatchedAuthenticatorWebAuthnStageRequest;
exports.PatchedAuthenticatorWebAuthnStageRequestFromJSON = PatchedAuthenticatorWebAuthnStageRequestFromJSON;
exports.PatchedAuthenticatorWebAuthnStageRequestFromJSONTyped = PatchedAuthenticatorWebAuthnStageRequestFromJSONTyped;
exports.PatchedAuthenticatorWebAuthnStageRequestToJSON = PatchedAuthenticatorWebAuthnStageRequestToJSON;
exports.PatchedAuthenticatorWebAuthnStageRequestToJSONTyped = PatchedAuthenticatorWebAuthnStageRequestToJSONTyped;
const UserVerificationEnum_1 = require("./UserVerificationEnum");
const AuthenticatorAttachmentEnum_1 = require("./AuthenticatorAttachmentEnum");
const ResidentKeyRequirementEnum_1 = require("./ResidentKeyRequirementEnum");
const FlowSetRequest_1 = require("./FlowSetRequest");
/**
 * Check if a given object implements the PatchedAuthenticatorWebAuthnStageRequest interface.
 */
function instanceOfPatchedAuthenticatorWebAuthnStageRequest(value) {
    return true;
}
function PatchedAuthenticatorWebAuthnStageRequestFromJSON(json) {
    return PatchedAuthenticatorWebAuthnStageRequestFromJSONTyped(json, false);
}
function PatchedAuthenticatorWebAuthnStageRequestFromJSONTyped(json, ignoreDiscriminator) {
    if (json == null) {
        return json;
    }
    return {
        'name': json['name'] == null ? undefined : json['name'],
        'flowSet': json['flow_set'] == null ? undefined : (json['flow_set'].map(FlowSetRequest_1.FlowSetRequestFromJSON)),
        'configureFlow': json['configure_flow'] == null ? undefined : json['configure_flow'],
        'friendlyName': json['friendly_name'] == null ? undefined : json['friendly_name'],
        'userVerification': json['user_verification'] == null ? undefined : (0, UserVerificationEnum_1.UserVerificationEnumFromJSON)(json['user_verification']),
        'authenticatorAttachment': json['authenticator_attachment'] == null ? undefined : (0, AuthenticatorAttachmentEnum_1.AuthenticatorAttachmentEnumFromJSON)(json['authenticator_attachment']),
        'residentKeyRequirement': json['resident_key_requirement'] == null ? undefined : (0, ResidentKeyRequirementEnum_1.ResidentKeyRequirementEnumFromJSON)(json['resident_key_requirement']),
        'deviceTypeRestrictions': json['device_type_restrictions'] == null ? undefined : json['device_type_restrictions'],
        'maxAttempts': json['max_attempts'] == null ? undefined : json['max_attempts'],
    };
}
function PatchedAuthenticatorWebAuthnStageRequestToJSON(json) {
    return PatchedAuthenticatorWebAuthnStageRequestToJSONTyped(json, false);
}
function PatchedAuthenticatorWebAuthnStageRequestToJSONTyped(value, ignoreDiscriminator = false) {
    if (value == null) {
        return value;
    }
    return {
        'name': value['name'],
        'flow_set': value['flowSet'] == null ? undefined : (value['flowSet'].map(FlowSetRequest_1.FlowSetRequestToJSON)),
        'configure_flow': value['configureFlow'],
        'friendly_name': value['friendlyName'],
        'user_verification': (0, UserVerificationEnum_1.UserVerificationEnumToJSON)(value['userVerification']),
        'authenticator_attachment': (0, AuthenticatorAttachmentEnum_1.AuthenticatorAttachmentEnumToJSON)(value['authenticatorAttachment']),
        'resident_key_requirement': (0, ResidentKeyRequirementEnum_1.ResidentKeyRequirementEnumToJSON)(value['residentKeyRequirement']),
        'device_type_restrictions': value['deviceTypeRestrictions'],
        'max_attempts': value['maxAttempts'],
    };
}
//# sourceMappingURL=PatchedAuthenticatorWebAuthnStageRequest.js.map