"use strict";
/* tslint:disable */
/* eslint-disable */
/**
 * authentik
 * Making authentication simple.
 *
 * The version of the OpenAPI document: 2025.6.3
 * Contact: hello@goauthentik.io
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.instanceOfContextualFlowInfo = instanceOfContextualFlowInfo;
exports.ContextualFlowInfoFromJSON = ContextualFlowInfoFromJSON;
exports.ContextualFlowInfoFromJSONTyped = ContextualFlowInfoFromJSONTyped;
exports.ContextualFlowInfoToJSON = ContextualFlowInfoToJSON;
exports.ContextualFlowInfoToJSONTyped = ContextualFlowInfoToJSONTyped;
const ContextualFlowInfoLayoutEnum_1 = require("./ContextualFlowInfoLayoutEnum");
/**
 * Check if a given object implements the ContextualFlowInfo interface.
 */
function instanceOfContextualFlowInfo(value) {
    if (!('cancelUrl' in value) || value['cancelUrl'] === undefined)
        return false;
    if (!('layout' in value) || value['layout'] === undefined)
        return false;
    return true;
}
function ContextualFlowInfoFromJSON(json) {
    return ContextualFlowInfoFromJSONTyped(json, false);
}
function ContextualFlowInfoFromJSONTyped(json, ignoreDiscriminator) {
    if (json == null) {
        return json;
    }
    return {
        'title': json['title'] == null ? undefined : json['title'],
        'background': json['background'] == null ? undefined : json['background'],
        'cancelUrl': json['cancel_url'],
        'layout': (0, ContextualFlowInfoLayoutEnum_1.ContextualFlowInfoLayoutEnumFromJSON)(json['layout']),
    };
}
function ContextualFlowInfoToJSON(json) {
    return ContextualFlowInfoToJSONTyped(json, false);
}
function ContextualFlowInfoToJSONTyped(value, ignoreDiscriminator = false) {
    if (value == null) {
        return value;
    }
    return {
        'title': value['title'],
        'background': value['background'],
        'cancel_url': value['cancelUrl'],
        'layout': (0, ContextualFlowInfoLayoutEnum_1.ContextualFlowInfoLayoutEnumToJSON)(value['layout']),
    };
}
//# sourceMappingURL=ContextualFlowInfo.js.map