"use strict";
/* tslint:disable */
/* eslint-disable */
/**
 * authentik
 * Making authentication simple.
 *
 * The version of the OpenAPI document: 2025.6.3
 * Contact: hello@goauthentik.io
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.instanceOfUserLoginChallengeResponseRequest = instanceOfUserLoginChallengeResponseRequest;
exports.UserLoginChallengeResponseRequestFromJSON = UserLoginChallengeResponseRequestFromJSON;
exports.UserLoginChallengeResponseRequestFromJSONTyped = UserLoginChallengeResponseRequestFromJSONTyped;
exports.UserLoginChallengeResponseRequestToJSON = UserLoginChallengeResponseRequestToJSON;
exports.UserLoginChallengeResponseRequestToJSONTyped = UserLoginChallengeResponseRequestToJSONTyped;
/**
 * Check if a given object implements the UserLoginChallengeResponseRequest interface.
 */
function instanceOfUserLoginChallengeResponseRequest(value) {
    if (!('rememberMe' in value) || value['rememberMe'] === undefined)
        return false;
    return true;
}
function UserLoginChallengeResponseRequestFromJSON(json) {
    return UserLoginChallengeResponseRequestFromJSONTyped(json, false);
}
function UserLoginChallengeResponseRequestFromJSONTyped(json, ignoreDiscriminator) {
    if (json == null) {
        return json;
    }
    return {
        'component': json['component'] == null ? undefined : json['component'],
        'rememberMe': json['remember_me'],
    };
}
function UserLoginChallengeResponseRequestToJSON(json) {
    return UserLoginChallengeResponseRequestToJSONTyped(json, false);
}
function UserLoginChallengeResponseRequestToJSONTyped(value, ignoreDiscriminator = false) {
    if (value == null) {
        return value;
    }
    return {
        'component': value['component'],
        'remember_me': value['rememberMe'],
    };
}
//# sourceMappingURL=UserLoginChallengeResponseRequest.js.map