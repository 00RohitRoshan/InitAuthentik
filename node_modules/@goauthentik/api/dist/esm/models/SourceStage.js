"use strict";
/* tslint:disable */
/* eslint-disable */
/**
 * authentik
 * Making authentication simple.
 *
 * The version of the OpenAPI document: 2025.6.3
 * Contact: hello@goauthentik.io
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.instanceOfSourceStage = instanceOfSourceStage;
exports.SourceStageFromJSON = SourceStageFromJSON;
exports.SourceStageFromJSONTyped = SourceStageFromJSONTyped;
exports.SourceStageToJSON = SourceStageToJSON;
exports.SourceStageToJSONTyped = SourceStageToJSONTyped;
const FlowSet_1 = require("./FlowSet");
/**
 * Check if a given object implements the SourceStage interface.
 */
function instanceOfSourceStage(value) {
    if (!('pk' in value) || value['pk'] === undefined)
        return false;
    if (!('name' in value) || value['name'] === undefined)
        return false;
    if (!('component' in value) || value['component'] === undefined)
        return false;
    if (!('verboseName' in value) || value['verboseName'] === undefined)
        return false;
    if (!('verboseNamePlural' in value) || value['verboseNamePlural'] === undefined)
        return false;
    if (!('metaModelName' in value) || value['metaModelName'] === undefined)
        return false;
    if (!('source' in value) || value['source'] === undefined)
        return false;
    return true;
}
function SourceStageFromJSON(json) {
    return SourceStageFromJSONTyped(json, false);
}
function SourceStageFromJSONTyped(json, ignoreDiscriminator) {
    if (json == null) {
        return json;
    }
    return {
        'pk': json['pk'],
        'name': json['name'],
        'component': json['component'],
        'verboseName': json['verbose_name'],
        'verboseNamePlural': json['verbose_name_plural'],
        'metaModelName': json['meta_model_name'],
        'flowSet': json['flow_set'] == null ? undefined : (json['flow_set'].map(FlowSet_1.FlowSetFromJSON)),
        'source': json['source'],
        'resumeTimeout': json['resume_timeout'] == null ? undefined : json['resume_timeout'],
    };
}
function SourceStageToJSON(json) {
    return SourceStageToJSONTyped(json, false);
}
function SourceStageToJSONTyped(value, ignoreDiscriminator = false) {
    if (value == null) {
        return value;
    }
    return {
        'name': value['name'],
        'flow_set': value['flowSet'] == null ? undefined : (value['flowSet'].map(FlowSet_1.FlowSetToJSON)),
        'source': value['source'],
        'resume_timeout': value['resumeTimeout'],
    };
}
//# sourceMappingURL=SourceStage.js.map